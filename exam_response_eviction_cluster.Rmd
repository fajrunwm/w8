...
The null hypothesis that I am going to test empirically is that there is no relationship 
  * with points ==> by doing PPA/PPP
  * densities of community districts ==> by doing Spatial autocorrelation 
  * with other factors that across New York ==> by doing Spatial regression
...
```{r}
evictions_points <- read_csv(here::here("data", "Evictions.csv"), na= "NULL")

community_areas <- st_read(here::here("data", "geo_export_65fab86c-efea-4868-9ae3-df21d21e1c3a.shp"))
```
...
This has reduced it to 60 points, if i remove the legal possession/eviction line then it's around 2,000
...
community_nb.lw <- community_nb %>%
  nb2listw(., style="W")
```
### Second: Spatial autocorrelation
